		...
		$(".assign_me").click(function(){
			...
			$.post('<c:url value="/task/assignMe"/>',{id:taskID},function(result){
				...
				if(result.code!='OK'){
					...
					var assignee = '<a class="a-tooltip" href="<c:url value="/user?id=${user.id}"/>" title="${user}"><img data-src="holder.js/40x40"	style="height: 40px; padding-right: 5px;" src="<c:url value="/userAvatar/${user.id}"/>" />'
									+'</a>';
